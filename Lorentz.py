...
videowriter = cv.VideoWriter("Lorentz.mp4", fourcc, 30, (size, size))  # 创建一个写入视频对象

for iter in np.arange(1,100,0.3):
    #微分方程起点
    x = 0
    y = 1
    z = 0
    ...
    for i in range(100000):
        ...
        trace_y.append((b * trace_x[-1] - trace_y[-1] - trace_x[-1] * trace_z[-1]) * delta_t + trace_y[-1])
        trace_z.append((trace_x[-1] * trace_y[-1] - c * trace_z[-1]) * delta_t + trace_z[-1])
